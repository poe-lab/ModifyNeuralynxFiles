function fixEventFileTimestamps02182015
working_dir = pwd;
...
[EventFilename, EventFilePath] = uigetfile({'*.nev',...
            'Pick an event file to be time shifted before being joined.'},'Select Event File');
nevFile = fullfile(EventFilePath, EventFilename);  
[Timestamps, EventIDs, TTLs, Extras, EventStrings, Header] =...
    Nlx2MatEV(nevFile, [1 1 1 1 1], 1, 1, [] );


fixedNevFilename = ['fixedTimestamps' EventFilename];
nevFile = fullfile(EventFilePath, fixedNevFilename);        
Mat2NlxEV(nevFile, 0, 1, [], [1 1 1 1 1], Timestamps, EventIDs, TTLs, Extras, EventStrings, Header);        


[vtFilename, vtFilePath] = uigetfile({'*.nvt',...
            'Pick VT file to be time shifted before being joined.'},'Select VT File');
vtFile = fullfile(vtFilePath, vtFilename);  
[Timestamps, X, Y, Angles, Targets, Points, Header] = Nlx2MatVT(vtFile, [1 1 1 1 1 1], 1, 1, [] );
fixedVtFilename = ['fixedTimestamps' vtFilename];
vtFile = fullfile(vtFilePath, fixedVtFilename);        
Mat2NlxVT(vtFile, 0, 1, [], [1 1 1 1 1 1], Timestamps, X, Y, Angles, Targets, Points, Header);        
        
%Fam Timestamps - (firstTimestampOfRevNtt = 5473695799) - +18305        
