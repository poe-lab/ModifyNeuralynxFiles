function downsampleClosestto1kHzCSC_v11022017
...
if downSampFactor == 1
    % If the sampling rate is as close to 1 kHz as possible, do nothing.
else    
    ...
    if ~isempty(T)
        ...
        CSCFilename = ['FillGaps '  CSCFilename];
    end
    clear T A
    ...
    clear targ targIdx
    HighFreqCut = str2double(strrep(Header{HighFreqCutIdx,1}, '-DspHighCutFrequency ', '')); %#ok<FNDSB>
    ...
    fixedNttFilename = ['SampleRate' num2str(newFs) 'Hz_' CSCFilename ];
    ...
    %% Save the results to a ne Neuralynx continuous file (.NCS):
